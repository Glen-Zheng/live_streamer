...
size_t get_magic_number(FILE *png, U8 *magic_number_buf) {
    size_t magic_number_bytes_read = fread(magic_number_buf, 1, PNG_SIG_SIZE, png);

    return magic_number_bytes_read;
}

int is_png(U8 *buf, size_t n) {
    if (n != PNG_SIG_SIZE) {
        fprintf(stderr, "Expected buffer to check if is_png to be 8 bytes (FAILURE)");
    }
    U8 png_magic_number[] = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A};
    if (memcmp(buf, png_magic_number, n) == 0) { // It's a PNG
        return 0; 
    }
    return -1;
}
...
int get_png_data_IHDR(struct data_IHDR *out, FILE *fp, long offset, int whence) {
    ...
    size_t bytes_read = fread(out, 1, DATA_IHDR_SIZE, fp);

    if (bytes_read != DATA_IHDR_SIZE) {
        fprintf(stderr, "Num bytes read was %zu, expected %d (FAILURE)", bytes_read, DATA_IHDR_SIZE); //%zu to print a size_t type var
    }
    fseek(fp, 0, SEEK_SET); //reset fp's file position pointer
